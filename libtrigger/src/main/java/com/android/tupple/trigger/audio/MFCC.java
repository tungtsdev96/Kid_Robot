package com.android.tupple.trigger.audio;
...
public class MFCC {
    ...
    private final static int hop_length = 160;
    ...
    public double[][] processTemp(double[] doubleInputBuffer) {
        ...
        for (int i = 0; i < melBasis.length; i++) {
            for (int j = 0; j < spectro[0].length; j++) {
                for (int k = 0; k < melBasis[0].length; k++) {
                    melS[i][j] += melBasis[i][k] * Math.abs(spectro[k][j]);
                }
                melS[i][j] = Math.pow(melS[i][j], 2);
            }
        }
        return melS;
    }
    ...
    private double[][] powerToDb(double[][] melS) {
        ...
        for (int i = 0; i < melS.length; i++) {
            for (int j = 0; j < melS[0].length; j++) {
                ...
                magnitude = melS[i][j];
//                if (magnitude > 1e-10){
                log_spec[i][j] = 10.0 * log10(Math.max(1E-10, magnitude));
//                }else{
                log_spec[i][j] -= 10.0 * log10(Math.max(1E-10, ref_v));
//                }
